 ...
 */
extension Double {
    /*: underneath, this is really a function `(Double) -> String` */
    var toString: String { "\(self)" }
}
extension String {
    /*: underneath, this is really a function `(String) -> String` */
    var encloseInSpaces: String { "   \(self)   " }
}

extension Int {
    /*: underneath, this is really a function `(Int) -> Double` */
...
flipYetAnother(14)
/*: Infix form: `+` is a function, just in "infix" form */
14 + 13
...
/*: Now lets introduce an `apply` function as reversed invocation
 Note: when marked with @inlinable the compiler reduces apply(a, f) to precisely f(a)
 */
func apply<A, R>(
    _ a: A,
    _ f: (A) -> R
