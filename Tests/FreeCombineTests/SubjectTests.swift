...
//                            return .more
        ...
        let r = await can.cancelAndAwaitResult()
        switch r {
            case let .failure(error): XCTFail("Cancellation return error: \(error)")
            case let .success(demand): XCTAssert(demand == .done, "Should have gotten .done")
        }
